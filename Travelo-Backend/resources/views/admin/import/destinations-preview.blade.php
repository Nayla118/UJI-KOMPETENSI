...
@section('title', 'Preview Destinations Import - Travelo Admin')
@section('header', 'Preview Destinations Import')

@section('content')
<div class="space-y-6">
    <!-- Back Link -->
    <a href="{{ route('admin.import.destinations') }}" class="inline-flex items-center gap-2 text-primary hover:text-blue-700">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <p class="text-gray-600 mb-4">Ready to import {{ $results['total_rows'] }} destinations. Click the button below to proceed with the import.</p>

        <form action="{{ route('admin.import.destinations.process') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.import.destinations') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-2.5 rounded-lg font-semibold transition-colors">
                    Cancel
                </a>
            </div>
        </form>
